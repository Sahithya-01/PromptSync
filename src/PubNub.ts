export default class PubNub {
  ...
  constructor(url?: string, protocols?: string) {
    ...
    this.url =
      url ||
      `wss://v6.pubnub3.com?subscribeKey=${
        import.meta.env.VITE_PUBNUB_SUBSCRIBE_KEY
      }&publishKey=${import.meta.env.VITE_PUBNUB_PUBLISH_KEY}&channel=pubnub`
...
const defaultSubkey: string = import.meta.env.VITE_PUBNUB_SUBSCRIBE_KEY
const defaultPubkey: string = import.meta.env.VITE_PUBNUB_PUBLISH_KEY
...
PUBNUB.subscribe = (setup: Setup = {}): AsyncGenerator<any, void, unknown> => {
  ...
  let messages: (m: any, jsonmsg?: any) => void =
    setup.messages ?? PUBNUB.messages ?? ((m: any) => m)
  let connect: (c: any) => void =
    setup.connect ?? PUBNUB.connect ?? ((c: any) => c)
  ...
  let filterExp: string = `${filter ? '&filter-expr=' : ''}${encodeURIComponent(
    filter
  )}`
  ...
  let promissory = (): Promise<any> =>
    new Promise((resolve) => (resolver = (data: object) => resolve(data)))
  ...
  if (channel.includes(',')) {
    throw new Error(
      'Only one channel is allowed. Comma symbol "," found in channel name.'
    )
  }
  ...
  async function startStream(): Promise<void> {
    ...
    try {
      ...
      reader =
        response.body?.getReader() as ReadableStreamDefaultReader<any> | null
    } catch (e) {
      return continueStream(1000)
    }
  ...
  async function readStream(): Promise<void> {
    let chunk: ReadableStreamReadResult<Uint8Array> | undefined = await reader
      ?.read()
      .catch((_) => {
        continueStream()
        return undefined
  ...
  function createSubscription(): AsyncGenerator<any, void, unknown> & {
    messages: Function
...
// PubNub History
PUBNUB.history = async (
  setup: Setup = {}
): Promise<Array<Array<any>> | false> => {
  let subkey: string = setup.subkey ?? PUBNUB.subscribeKey ?? defaultSubkey
  let channel: string = setup.channel ?? PUBNUB.channel ?? defaultChannel
  let uuid: string = setup.uuid ?? PUBNUB.uuid ?? defaultUUID
  let origin: string = setup.origin ?? PUBNUB.origin ?? defaultOrigin
...
PUBNUB.publish = async (setup: Setup = {}): Promise<Response | false> => {
  ...
  let params: string = `uuid=${uuid}&auth=${authkey}&meta=${encodeURIComponent(
